...
class Jobs extends Component {
  ...
  getJobsList = async () => {
    ...
    const jwtToken = Cookies.get('jwt_token')
    const {activeEmployeeType, salaryRange, searchInputValue} = this.state
    const apiUrl = `https://apis.ccbp.in/jobs?employment_type=${activeEmployeeType}&minimum_package=${salaryRange}&search=${searchInputValue}`
    const options = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
  ...
  changeActiveEmployeeType = e => {
    const {activeEmployeeType} = this.state
    const inputNotInList = activeEmployeeType.filter(
      each => each === e.target.id,
    )
    if (inputNotInList.length === 0) {
      this.setState(
        prevState => ({
          activeEmployeeType: [...prevState.activeEmployeeType, e.target.id],
        }),
        this.getJobsList,
      )
    } else {
      const filteredData = activeEmployeeType.filter(
        each => each !== e.target.id,
      )
      this.setState({activeEmployeeType: filteredData}, this.getJobsList)
    }
  }
      ...
      {employmentTypesList.map(each => (
        <li key={each.employmentTypeId} className="li-item">
          <input
            ...
            onChange={this.changeActiveEmployeeType}
          />
